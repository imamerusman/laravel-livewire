...
use App\Events\FeatureConsumed;
use App\Events\FeatureUnconsumed;
use Carbon\Carbon;
...
class PlanSubscription extends Model
{
    ...
    public function consumeFeature(string $featureCode, float $amount): bool
    {
        ...
        if (! $usage) {
            $usage = $this->usages()->save(new PlanSubscriptionUsage([
                'code' => $featureCode,
                'used' => 0,
            ]));
        }
    ...
    public function unconsumeFeature(string $featureCode, float $amount): bool
    {
        ...
        $used = ($feature->isUnlimited()) ? 0 : max($usage->used - $amount, 0);
        // Ensure it doesn't go negative
        $remaining = ($feature->isUnlimited()) ? -1 : max($feature->limit - $used, 0);
