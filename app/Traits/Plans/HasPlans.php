...
use App\Events\ExtendSubscription;
use App\Events\ExtendSubscriptionUntil;
use App\Events\NewSubscription;
use App\Events\NewSubscriptionUntil;
use App\Events\UpgradeSubscription;
use App\Events\UpgradeSubscriptionUntil;
...
trait HasPlans
{
     ...
     */
    public function getPlanSubscriptions(): MorphMany
    {
    ...
    {
        return $this->getPlanSubscriptions()
    ...
    public function lastActiveSubscription(): ?PlanSubscription
    {
        ...
        return $this->getPlanSubscriptions()->latest('starts_on')->paid()->notCancelled()->first();
    }
    ...
    public function lastSubscription(): Model|MorphMany|null
    {
        ...
        return $this->getPlanSubscriptions()->latest('starts_on')->first();
    }
    ...
    {
        return $this->getPlanSubscriptions()->latest('starts_on')
            ->notCancelled()
            ->unpaid()
            ->first();
    }
    ...
    {
        return (bool) ($this->getPlanSubscriptions()->count() > 0);
    }
    ...
    public function subscribeTo(Plan $plan, int $duration = 30, bool $isRecurring = true): Model|bool
    {
        ...
        $subscription = $this->getPlanSubscriptions()->save(new $subscriptionModel([
            'plan_id' => $plan->id,
        ...
        if ($this->subscriptionPaymentMethod == 'stripe') {
            try {
                ...
                event(new \App\Events\Stripe\ChargeSuccessful($this, $subscription, $stripeCharge));
            } catch (\Exception $exception) {
                event(new \App\Events\Stripe\ChargeFailed($this, $subscription, $exception));
            }
        }
    ...
    public function subscribeToUntil(Plan $plan, DateTime|string $date, bool $isRecurring = true): Model|bool
    {
        ...
        $subscription = $this->getPlanSubscriptions()->save(new PlanSubscription([
            'plan_id' => $plan->id,
            'starts_on' => Carbon::now()->subSeconds(1),
            'expires_on' => $date,
            'cancelled_on' => null,
